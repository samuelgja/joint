...
export function readLayerRecords(
  ...
  for (const layer of layers) {
    ...
    // back to a scan only when the order changed. ponytail: O(L²) on a full
    // reorder; an id index pays off past ~50 layers.
...
export function reconcileLayers(
  ...
  // move. ponytail: O(L) moves, LCS-minimal moves if diagrams grow past a handful of layers.
...
export function withLayerPatch(
  ...
  // ponytail: O(L) rebuild on the write path — L is a handful of layers.
