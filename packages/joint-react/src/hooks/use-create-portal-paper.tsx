...
export function useCreatePortalPaper(
  ...
  // Subscribe to the set of cell ids — the shallow-equal selector keeps the same
  // array reference across data-only commits, so this only re-renders when cells
  // are added or removed. Partition the ids by type for element / link portals.
  const allCellIds = useCells(
    (cells) => cells.map((cell) => cell.id as CellId),
