...
>(options: GraphProjectionState<Element, Link>) {
  ...
  return {
    ...
    updateGraph(update: UpdateGraphOptions<Element, Link>) {
      ...
      // Set allocation. `pendingAdded.size` covers cells not yet committed.
      // Steady state for controlled components re-pushing the same set.
      if (cells.getSnapshot().length + pendingAdded.size > cellIds.length) {
