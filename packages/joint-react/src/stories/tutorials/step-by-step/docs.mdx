...
- **Diagram:** React context provider for managing the graph state.
- **Diagram.View:** The main rendering component for nodes and links.
...
## 4. Setting up the Diagram context

Wrap your app with the `Diagram` component to provide the graph context to all child components.
...
```tsx
<Diagram.View elements={elements} links={links} />
```
...
The `Diagram.View` component renders your graph. It provides props for customizing the rendering and handling interactions.
...
- **Diagram:** React context for graph data.
- **Diagram.View:** Renders the graph visually.
...
## 11. Multiple views in a single Diagram

You can render multiple `Diagram.View` instances inside one `Diagram` to create features like a minimap, a read-only overview, or separate layers. When using multiple views in development, assign a unique `id` to each view.
...
export function MultiViews() {
  return (
    <Diagram elements={elements}>
      ...
      <Diagram.View id="main" width={800} height={500} />
...
- Share the same `elements`/`links` via the parent `Diagram`.
...
Use an internal hook to subscribe to a named view from anywhere under `Diagram`. This is handy for utilities colocated outside the view subtree.

```tsx
import { useEffect } from 'react';
// import { useDiagramView } from '@joint/react' // ensure it is exported in your setup

function FitToContent() {
  const view = useDiagramView('main');
  useEffect(() => {
...
Attach a `ref` directly to `Diagram.View` to get `paper`/`graph` from the resolved config.
...
export function WithRef() {
  const viewRef = useRef<ViewConfig | null>(null);
  useEffect(() => {
  ...
  return (
    <Diagram elements={elements}>
      <Diagram.View id="zoomed" ref={viewRef} />
    </Diagram>
  );
}
```
...
## 13. Small real‑world patterns

### a. Canvas + Minimap

```tsx
function DiagramWithMinimap() {
  return (
    <Diagram elements={elements} links={links}>
      <Diagram.View id="main" width="100%" height={400} />
      <div style={{ position: 'absolute', right: 16, bottom: 16 }}>
        <Diagram.View id="mini" width={180} height={120} interactive={false} scale={0.2} />
      </div>
    </Diagram>
  );
}
...
### b. Read‑only preview next to an editor
...
function EditorWithPreview() {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
      <Diagram elements={elements} links={links}>
        <Diagram.View id="editable" />
      </Diagram>
      <Diagram elements={elements} links={links}>
        <Diagram.View id="preview" interactive={false} />
      </Diagram>
    </div>
  );
}
...
function HtmlNodes() {
  return (
    <Diagram elements={elements}>
      <Diagram.View useHTMLOverlay renderElement={renderElement} />
    </Diagram>
  );
}
```
